 ...
 * Task: T034
 *
 * Maps frontmatter fields from Astro electricity-rate MDX files to Payload CMS field structure.
 * Handles all 15 discovered frontmatter fields including nested fields (seo, hero).
 *
 * Frontmatter Fields (15 total):
 * - draft, cityName, seo_title, cp_hero_heading_line_1, cp_hero_heading_line_2
 * - title, wp_slug, wp_post_id, seo_meta_desc, target_keyword, pubDate
 * - updatedDate, wp_author, cp_hero_cta_text, cityRef
 *
 * Payload Fields:
 * - title, slug, status, cityName, cityRef, wordpressSlug, wpPostId, wpAuthor
...
export interface RateFrontmatter {
  title?: string
...
/**
 * Map rate frontmatter to Payload field structure
 *
 ...
 *
 * @example
 * const data = mapRateFields(
 *   { title: 'Houston Electricity Rates', draft: false, cityName: 'Houston', pubDate: '2024-01-01' },
 *   '/path/to/houston-electricity-rates/index.mdx',
 *   '/path/to/electricity-rates'
 * )
 */
export async function mapRateFields(
  frontmatter: RateFrontmatter,
  filePath: string,
  sourceDir: string,
  mdxContent?: string
): Promise<RateData> {
  ...
  const data: RateData = {
    ...
    cityName: cleanString(frontmatter.cityName) || 'Unknown City',
    publishedAt: parseDate(frontmatter.pubDate),

    // Optional city reference
    cityRef: cleanString(frontmatter.cityRef),
  ...
  // Build Hero nested object
  const heroLine1 = frontmatter.cp_hero_heading_line_1
  const heroLine2 = frontmatter.cp_hero_heading_line_2
  const heroCtaText = frontmatter.cp_hero_cta_text
...
/**
 * Validate required rate fields are present
 *
 * @param data - Mapped rate data
 * @returns Array of missing field names (empty if valid)
 */
export function validateRateData(data: RateData): string[] {
